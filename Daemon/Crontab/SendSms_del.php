            ...
            foreach ( $message as $messageinfo ) {
                ...
                if (intval($rs)) {
                    ...
                    if(in_array($rs,$statusarr)){
                        $emailObj = ClsFactory::Create('@.Common_new.WmwEmail');
                        $send = $emailObj->send('zhaosheng@wmw.cn',$this->checkstatus($rs));
                        $flag = false;
                        break;
                    }
                    if($rs!="100"){
                    }
                    $result = $mUnicomInterface->modifySmsSend($dataarr, $messageinfo['sms_send_id']);
                    if(!$result){
                        $flag = false;
                        break;
                    }
